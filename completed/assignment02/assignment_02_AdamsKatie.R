...
## Use `setwd()` if needed
setwd("C:/Users/kadams/OneDrive - Suncor Energy Inc/DSC Masters Program/GitHub/DSC520/Assignments/dsc520/data")

## Load the file `data/tidynomicon/person.csv` to `person_df1` using `read.csv`
## Examine the structure of `person_df1` using `str()`
person_df1 <- read.csv("tidynomicon/person.csv", header= TRUE)
str(person_df1)
...
person_df2 <- read.csv("tidynomicon/person.csv", header= TRUE, stringsAsFactors = FALSE)
str(person_df2)

## Read the file `data/scores.csv` to `scores_df`
## Display summary statistics using the `summary()` function
scores_df <- read.csv("scores.csv", header = TRUE)
summary(scores_df)
...
voter_turnout_df1 <- read_excel("G04ResultsDetail2004-11-02.xls", 2, skip = 1)
...
voter_turnout_df2 <- read_excel("G04ResultsDetail2004-11-02.xls", 2, skip = 2, col_names = c("ward_precint", "ballots_cast", "registered_voters", "voter_turnout"))
...
scores_json <- toJSON(scores_df, pretty=TRUE)
