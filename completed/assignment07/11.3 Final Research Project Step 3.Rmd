...
rm_bat_5years <-  lm(Wins ~ R.G + PA + R + Doubles + RBI + BB + OBP + SLG + OPS + OPS. + TB + SF, data = df_bat_5years) #Used the variables with the highest correlation to Wins in the correlation matrix
rm_bat_5years
...
1. Major League Baseball (MLB) has a spending problem, where they spend too much money on players because of a flawed assumption. That flawed assumption is that buying (i.e. offering contracts) players who are perceived "better" (i.e. players like Johnny Damon) will buy the team more wins. When in actuality, buying runs (i.e. exclusively looking at the players who have historically made a high number of runs in comparison to other plays) will buy the team wins. To understand what MLB Team Standing stats are correlated with the number of team wins, a correlation matrix was created, and it identified that Wins were postively correlated with the follow MLB Team Standing varibles: R/G (Runs Scored Per Game), PA (Plate Apperances), R (Runs Scored/Allowed), 2B (Doubles), HR (Home Runs), RBI (Runs Batted In), BB (Bases on Balls/Walks), OBP (On Base Percentage), SLG (Slugging Percentage), OPS (On Base+Slugging Percentage), OPS+ (On Base+Slugging Percentage adjusted to player's ballpark), and TB (Total Bases). These highly correlated variables were the only ones added to the linear regression model, where it was found in the summary table that the variable BB (Bases on Balls/Walks) is statistically significant, but OPS has a larger positive effect on the number of Wins, where SLG and OBP have a larger negative effect on number of wins.

2. Top Correlated Variables to Number of Wins
-            R.G - Wins  0.823618
-             PA - Wins  0.783488
-              R - Wins  0.817370
-            X2B - Wins  0.604298
...
1. From the 2016-2020 MLB Team Standings dataset, a correlation matrix was created to determine which variables were postively correlated with number of wins. From that, the top positively correlated variables were inputted into a linear regression model to calculate the significance codes for each variable, and determine which dataset predictors are significant in the linear regression model (statistically significant predictors). This addresses the problem of what MLB Team Standing variable predictors are statistically significant in relation to number of wins. 

## Analysis
### Summarize the interesting insights that your analysis provided. 
1. Positive Correlation: I found that Number of Wins variable is highly correlated to the following variables in the dataset: R/G (Runs Scored Per Game), PA (Plate Apperances), R (Runs Scored/Allowed), 2B (Doubles), HR (Home Runs), RBI (Runs Battled In), BB (Bases on Balls/Walks), OBP (On Base Percentage), SLG (Slugging Percentage), OPS (On Base+Slugging Percentage), OPS+ (On Base+Slugging Percentage adjusted to player's ballpark), and TB (Total Bases). 
2. Negative Correlation: I also found the Number of Wins variable is not highly correlated to AvgNumPlayers (Average number of players used in games), 3B (Triples), CS (Caught Steeling), or SO (Strikeouts).
3. Statistically Significant: Within the linear model, BB (Bases on Balls/Walks) is statistically significant. In looking at the estimates of the model, OPS has a larger positive effect on the number of Wins, where SLG and OBP have a larger negative effect on number of wins. 

## Implications
### Summarize the implications to the consumer (target audience) of your analysis. 
1. The implications of these findings to the MLB team scouts looking to recruit new baseball players is that they should look generally at the highly correlated variables of the model: R/G (Runs Scored Per Game), PA (Plate Apperances), R (Runs Scored/Allowed), 2B (Doubles), HR (Home Runs), RBI (Runs Batted In), BB (Bases on Balls/Walks), OBP (On Base Percentage), SLG (Slugging Percentage), OPS (On Base+Slugging Percentage), OPS+ (On Base+Slugging Percentage adjusted to player's ballpark), and TB (Total Bases). Also, they should highly focus of BB (Bases on Balls/Walks) as it is highly significant within the linear model. 
2. For the 2021 MLB season, we can expect that the variable OPS will have a larger affect on the number of Wins as it is statistically significant, where SLG and OBP have a negative effect on the number of wins.
